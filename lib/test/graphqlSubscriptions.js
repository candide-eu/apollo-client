...
describe('GraphQL Subscriptions', function () {
    ...
    it('should receive multiple results for a subscription with Apollo Link', function (done) {
        var numResults = 0;
        var expectedData = [
            'Dahivat Pandya',
            'Vyacheslav Kim',
            'Changping Chen',
            'Amanda Liu',
        ].map(function (name) { return ({ user: { name: name } }); });
        var queryInfo = {
            request: defaultSub1.request || {},
            results: expectedData.slice(),
        };
        var link = ApolloLink.from([
            function (operation) {
                return new Observable(function (observer) {
                    if (queryInfo.results) {
                        queryInfo.results.map(function (result) {
        ...
        var obs = client.subscribe(queryInfo.request);
        var sub = obs.subscribe({
            next: function (data) {
            ...
            error: console.log,
            complete: function () { return assert(false); },
        });
    });
    it('should unsubscribe properly with Apollo Link', function () {
        var numResults = 0;
        var readyToUnsub = [];
        var link = ApolloLink.from([
            function (operation) {
                return new Observable(function (observer) {
                    var index = readyToUnsub.length;
        ...
        var obs0 = client.subscribe(defaultSub1.request);
        var obs1 = client.subscribe(defaultSub1.request);
        var subscription0 = obs0.subscribe({});
        var subscription1 = obs1.subscribe({});
        readyToUnsub[1] = true;
        subscription1.unsubscribe();
        readyToUnsub[0] = true;
        subscription0.unsubscribe();
        readyToUnsub.map(function (bool) { return assert(bool); });
    });
    it('should fire redux action and call result reducers', function (done) {
        ...
        var observableQuery = queryManager
            .watchQuery({
            query: query,
