...
describe('client', function () {
    ...
    it('should allow multiple queries with an apollo-link enabled network interface', function (done) {
        var query = (_a = ["\n      query people {\n        allPeople(first: 1) {\n          people {\n            name\n            __typename\n          }\n          __typename\n        }\n      }\n    "], _a.raw = ["\n      query people {\n        allPeople(first: 1) {\n          people {\n            name\n            __typename\n          }\n          __typename\n        }\n      }\n    "], gql(_a));
        var query2 = (_b = ["\n      query people {\n        people {\n          id\n        }\n      }\n    "], _b.raw = ["\n      query people {\n        people {\n          id\n        }\n      }\n    "], gql(_b));
        var data = {
            allPeople: {
        ...
        client.query({ query: query, variables: variables }).then(function (actualResult) {
            assert.deepEqual(actualResult.data, data);
            done1 = true;
            if (done2) {
                done();
        ...
        client.query({ query: query2, variables: variables }).then(function (actualResult2) {
    ...
    describe('cache-and-network fetchPolicy', function () {
        ...
        it('fetches from cache first, then network and does not have an unhandled error on network failure', function (done) {
            var networkInterface = mockNetworkInterface({
                request: { query: query },
            ...
            client.writeQuery({ query: query, data: initialData });
            var obs = client.watchQuery({
                query: query,
                fetchPolicy: 'cache-and-network',
            });
            var shouldFail = true;
            process.once('unhandledRejection', function (rejection) {
                if (shouldFail)
                    done(new Error('promise had an unhandledRejection'));
            });
            var count = 0;
            obs.subscribe({
                next: function (result) {
